...
function addGround() {
    ...
    for(var row = 0; row<3; row+=1) {
        ...
        for (var z = -params.tileLength; z < (params.tileLength * 2); z += params.tileLength){
            groundMesh = addingGroundMesh(count, xPos, z);
            count++;
...
function addingGroundMesh(index, xPos, z) {

    var groundGeometry = new THREE.PlaneGeometry(params.tileLength, params.tileLength, params.tileVertices, params.tileVertices);
...
function modifyingGroundMesh(index, xPos, z) {
    for( var i = 0; i < xPosition[index].length; i++){
        xPosition[index][i] = xPos;
        zPosition[index][i] = z;
    }
